...
import { ArrowLeft, Edit, Download, Send, FileText, Calendar, User, Loader2, CheckCircle, Receipt, Mail, MessageSquare, X } from 'lucide-react';
...
const QuoteDetail: React.FC<QuoteDetailProps> = ({ quote, onClose, onEdit }) => {
    ...
    const handleShareEmail = async () => {
        ...
        try {
            ...
            alert(`La cotización se guardó y el PDF se descargó. Se abrirá Gmail en una nueva pestaña con el mensaje precargado; adjunta manualmente el archivo "${pdfFileName}" antes de enviarlo.`);
    ...
    const handleShareWhatsApp = async () => {
        ...
        try {
            ...
            alert(`La cotización se guardó y el PDF se descargó. En la ventana de WhatsApp que se abrirá ahora, adjunta manualmente el archivo "${pdfFileName}".`);
    ...
    const handleConfirmApproval = async (
        ...
        try {
            ...
            // 2. Deduct inventory for physical products
            ...
            // 3. Generate Charge Note & register advance payment if applicable
            if (generateChargeNote) {
                const dueDate = new Date();
                dueDate.setDate(dueDate.getDate() + 30);

                const chargeNoteData = {
                    client_id: quote.prospectId,
                ...
                const newChargeNote = await addChargeNote(chargeNoteData, chargeNoteItems);

                if (newChargeNote && advanceAmount > 0 && accountId) {
                    await registerPayment({
                        documentId: newChargeNote.id,
                        accountId: accountId,
                        monto: advanceAmount,
                        fechaPago: new Date(),
                        metodoPago: paymentMethod as any,
                        referencia: `Anticipo cotización ${quote.numero}`,
                        notas: notes,
                        comprobanteUrl: ''
                    });
                }
            ...
            alert('Cotización aprobada correctamente' + (generateChargeNote ? ' y Nota de Cargo generada en Finanzas.' : '.'));
        } catch (error) {
