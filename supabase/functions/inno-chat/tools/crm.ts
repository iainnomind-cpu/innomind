...
export async function leerCotizaciones(supabase: SupabaseClient, workspaceId: string, platform: "track" | "crm_erp") {
  try {
    console.log(`[leerCotizaciones] Iniciando consulta. Workspace: ${workspaceId}, Plataforma: ${platform}`);
    if (platform === "track") {
      ...
      if (error) {
        console.error("[leerCotizaciones Track Error]:", error);
      ...
      console.log(`[leerCotizaciones CRM-ERP] Consultando tabla 'quotes' con filtro 'workspace'`);
      // Nota: Eliminamos created_at para evitar errores si la columna no existe (usamos fecha)
      const { data, error } = await supabase
        .from("quotes")
        .select("id, numero, fecha, vigencia, estado, total, subtotal, descuento, iva_total, items")
        .eq("workspace", workspaceId)
        .order("fecha", { ascending: false })
        .limit(15);

      if (error) {
        console.error("[leerCotizaciones CRM-ERP Error]:", error);
        return JSON.stringify({
          error: true,
          message: error.message,
          code: error.code,
      ...
      console.log(`[leerCotizaciones CRM-ERP] Consulta exitosa. Se encontraron ${adapted.length} cotizaciones.`);
      return JSON.stringify(adapted);
    } else {
      throw new Error(`Plataforma no admitida para leerCotizaciones: ${platform}`);
    }
  } catch (err: any) {
    console.error("[leerCotizaciones Fatal Error]:", err);
    return JSON.stringify({
      error: true,
      message: err.message || "Error fatal en la herramienta leerCotizaciones"
    });
  }
}
